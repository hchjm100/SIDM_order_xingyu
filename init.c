...
void init(void)
{
  ...
  //  2024.04.18: Set a switch of manual restarting ...

  char* keyword_ZXY = "snapshot_";
  char* pos_keyword_ZXY = strstr(All.InitCondFile, keyword_ZXY);
  if(pos_keyword_ZXY != NULL){
    char* NumCurrSnapStr = pos_keyword_ZXY + strlen(keyword_ZXY);
    int NumCurrSnap = atoi(NumCurrSnapStr);
    printf("\nZXY: \"snapshot\" detected, snap: %s\n", NumCurrSnapStr);
    printf("\nZXY: String to integer test: snap %d\n", NumCurrSnap);

    All.DisplaceOnZXY = 0;  // ZXY: Usually restarting should not displace ......
    All.SnapshotFileCount = NumCurrSnap; 
  }
...
/*
 * 2023.04.02 ZXY: Now run non-cosmological runs ......
...
if(All.DisplaceOnZXY){
  for(i = 0; i < NumPart; i++){
//  2024.04.19 ZXY: Orbit 4 start at 2.9 Gyr ...
    P[i].Pos[0] += 58.16;
    P[i].Pos[1] += 40.12;
    P[i].Pos[2] += 101.41;
