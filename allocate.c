...
/*
  All.BunchSizeSIDM =
    (All.BufferSize * 1024 * 1024) / (4 * sizeof(struct sidmdata_in));

  SIDMDataIn = (struct sidmdata_in *) CommBuffer;
  SIDMDataGet = SIDMDataIn + All.BunchSizeSIDM;
  SIDMDataOut = SIDMDataGet + All.BunchSizeSIDM;
  SIDMDataResult = SIDMDataOut + All.BunchSizeSIDM;
*/
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
/* cheaper: taking advantage that the In & Out have the same structure */
  All.BunchSizeSIDM =
    (All.BufferSize * 1024 * 1024) / (2 * sizeof(struct sidmdata_in));

  SIDMDataIn = (struct sidmdata_in *) CommBuffer;
  SIDMDataGet = SIDMDataIn + All.BunchSizeSIDM;
  SIDMDataOut = SIDMDataIn;
  SIDMDataResult = SIDMDataGet;
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
