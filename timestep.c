  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Ti_endstep == All.Ti_Current)
	{
	  ti_step = get_timestep(i, &aphys, 0);
          // ------ DY ------------------------
          /*
          if(P[i].Type == 1 && P[i].Pijmax>0.01){
             ndiv=(int)(P[i].Pijmax/0.01);
             if((P[i].Ti_endstep-P[i].Ti_begstep)>(2*ndiv)){
                if(ndiv<2) ndiv=2;
                ti_step = (P[i].Ti_endstep-P[i].Ti_begstep)/ndiv;
             }
          }
          else if(P[i].Type == 1 && P[i].Pijmax<0.0001 && P[i].Pijmax>0 && (P[i].Ti_endstep-P[i].Ti_begstep) < ti_step){
             ti_step = (P[i].Ti_endstep-P[i].Ti_begstep) << 1;
          }
          */
          //if(P[i].ID==17763) printf("\n\n@@@@@@@@@@@@@@@@@@@@@ ID=%d, Pijmax=%f, timestep=%d @@@@@@\n\n",P[i].ID,P[i].Pijmax,P[i].Ti_endstep-P[i].Ti_begstep); 
          //if(P[i].Ti_endstep!=0 && P[i].Pijmax>0.0005) printf("\n\n@@@@@@@@@@@@@@@@@@@@@ ID=%d, Pijmax=%f, tbeg=%d, tend=%d @@@@@@\n\n",P[i].ID,P[i].Pijmax,P[i].Ti_begstep,P[i].Ti_endstep); 
          //P[i].Pijmax=-1.0; // initialize for next synchronization
          // ------ DY ------------------------
